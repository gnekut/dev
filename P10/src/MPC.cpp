 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    */
    ...
    for (int t = 0; t < N; t++) {
      fg[0] += 5000 * CppAD::pow(vars[cte_s + t] - cte_ref, 2);   // Cross-track error (distance from polynomial)
      fg[0] += 5000 * CppAD::pow(vars[epsi_s + t] - epsi_ref, 2); // Heading error (difference in polynomial slope and car direction)
      fg[0] += 10 * CppAD::pow(vars[v_s + t] - v_ref, 2);       // Velocity error
    }
    // Adding steering and acceleration error/cost.
    for (int t = 0; t < N - 1; t++) {
      fg[0] += 10 * CppAD::pow(vars[delta_s + t], 2); // Steering angle penalty (straight vs. turning)
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += 50 * CppAD::pow(vars[delta_s + t + 1] - vars[delta_s + t], 2); // Steering angle change (sharp vs smooth turns)
      fg[0] += 1 * CppAD::pow(vars[a_s + t + 1] - vars[a_s + t], 2);          // Acceleration change (hard vs soft accelerations)
    }
    // =============================================================================
